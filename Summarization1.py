# pip install requests torch transformers in terminal
...
try:
    ...
    SUMMARIZATION_PIPE = pipeline(
        ...
        max_length=250,  #   250 tokens for shorter summaries
        min_length=100,  #  min length to be proportional to new max length
        #  Generation and rpeition paramters
        num_beams=8,              # summary quality
        no_repeat_ngram_size=4,   # repitition guard
        length_penalty=2.0,       # Encourages the model to generate a longer output
        do_sample=False
    )
...
def summarize_text_local(bill_text: str) -> str | None:
    ...
    if len(clean_text) > MAX_INPUT_CHARS:
        ...
        try:
            # Runs the final summarizer on the concatenated summaries
    ...
    # (for bills within the size limit)
    else:
        try:
...
def loop_through_bills():
    ...
    # Fetches all bills from the specified path
    ...
    # 2. converted to tuples for iteration
